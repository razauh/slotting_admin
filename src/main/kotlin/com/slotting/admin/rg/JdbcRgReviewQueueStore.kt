...
@Repository
class JdbcRgReviewQueueStore(private val jdbc: JdbcTemplate) : RgQueueStore {
    override fun findByIdempotency(tenantId: String, key: String) = jdbc.query(
        "select result_id, query_fingerprint, case_reference, state, claimed_by, claim_expires_at, server_version, occurred_at from admin_rg_review_result where tenant_id = ? and idempotency_key = ?",
        { rs, _ -> rs.toResult() }, tenantId, key
    ).firstOrNull()

    override fun findItem(tenantId: String, caseReference: String) = jdbc.query(
        "select case_reference, state, claimed_by, claim_expires_at, server_version from admin_rg_review_queue where tenant_id = ? and case_reference = ?",
        { rs, _ ->
            RgQueueItem(
                rs.getString("case_reference"),
                RgReviewState.valueOf(rs.getString("state")),
    ...
    override fun save(
        ...
        jdbc.update(
            ...
            result.resultId, tenantId, "RG_REVIEW", result.serverTime
        )
        jdbc.update(
    ...
    private fun ResultSet.toResult(): Pair<String, RgReviewResult> {
        val id = getObject("result_id", UUID::class.java)
        val item = RgQueueItem(
            getString("case_reference"),
            RgReviewState.valueOf(getString("state")),
            getString("claimed_by"),
            getTimestamp("claim_expires_at")?.toInstant(),
            getLong("server_version"),
        )
        return getString("query_fingerprint") to RgReviewResult(id, item, getTimestamp("occurred_at").toInstant(), "rg-review:$id")
    }
}
