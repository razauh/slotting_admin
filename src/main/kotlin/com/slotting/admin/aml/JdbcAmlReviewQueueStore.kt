...
@Repository
class JdbcAmlReviewQueueStore(private val jdbc: JdbcTemplate) : AmlQueueStore {
    override fun findByIdempotency(tenantId: String, key: String) = jdbc.query(
        "select result_id, query_fingerprint, case_reference, state, claimed_by, claim_expires_at, server_version, occurred_at from admin_aml_review_result where tenant_id = ? and idempotency_key = ?",
        { rs, _ -> rs.toResult() }, tenantId, key
    ).firstOrNull()

    override fun findItem(tenantId: String, caseReference: String) = jdbc.query(
        "select case_reference, state, claimed_by, claim_expires_at, server_version from admin_aml_review_queue where tenant_id = ? and case_reference = ?",
        { rs, _ ->
            AmlQueueItem(
                rs.getString("case_reference"),
                AmlReviewState.valueOf(rs.getString("state")),
                rs.getString("claimed_by"),
                rs.getTimestamp("claim_expires_at")?.toInstant(),
                rs.getLong("server_version"),
            )
        },
        tenantId, caseReference
    ).firstOrNull()
    ...
    override fun save(
        ...
        jdbc.update(
            "update admin_aml_review_queue set state = ?, claimed_by = ?, claim_expires_at = ?, server_version = ? where tenant_id = ? and case_reference = ? and server_version = ?",
        ...
        jdbc.update(
            "insert into admin_aml_review_result(result_id, tenant_id, case_reference, query_fingerprint, reason_code, state, claimed_by, second_approver_id, claim_expires_at, server_version, occurred_at, idempotency_key, correlation_id, causation_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        jdbc.update(
            ...
            result.resultId, tenantId, "AML_REVIEW", result.serverTime
        )
        jdbc.update(
    ...
    private fun ResultSet.toResult(): Pair<String, AmlReviewResult> {
        val id = getObject("result_id", UUID::class.java)
        val item = AmlQueueItem(
            getString("case_reference"),
            AmlReviewState.valueOf(getString("state")),
            getString("claimed_by"),
            getTimestamp("claim_expires_at")?.toInstant(),
            getLong("server_version"),
        )
        return getString("query_fingerprint") to AmlReviewResult(id, item, getTimestamp("occurred_at").toInstant(), "aml-review:$id")
    }
}
