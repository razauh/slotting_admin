...
@Repository
class JdbcGameProviderConfigStore(private val jdbc: JdbcTemplate) : GameProviderConfigStore {
    override fun findByIdempotency(tenantId: String, key: String) = jdbc.query(
        "select result_id, query_fingerprint, evidence_reference, provider_id, display_name, status, endpoint_url, masked_secret, incident_reference, server_version, occurred_at from admin_game_provider_config_result where tenant_id = ? and idempotency_key = ?",
        { rs, _ -> rs.toResult() }, tenantId, key
    ).firstOrNull()

    override fun findProvider(tenantId: String, providerId: String) = jdbc.query(
        "select provider_id, display_name, status, endpoint_url, masked_secret, incident_reference, server_version from admin_game_provider_config where tenant_id = ? and provider_id = ?",
        { rs, _ ->
            GameProviderConfig(
                providerId = rs.getString("provider_id"),
                displayName = rs.getString("display_name"),
                status = GameProviderStatus.valueOf(rs.getString("status")),
    ...
    override fun save(
        ...
        if (existing == null) {
            jdbc.update(
                "insert into admin_game_provider_config(tenant_id, provider_id, display_name, status, endpoint_url, secret_hash, masked_secret, incident_reference, server_version, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ...
            if (secretHash.isNotBlank()) {
                jdbc.update(
                    "update admin_game_provider_config set display_name = ?, status = ?, endpoint_url = ?, secret_hash = ?, masked_secret = ?, incident_reference = ?, server_version = ?, updated_at = ? where tenant_id = ? and provider_id = ? and server_version = ?",
                ...
                jdbc.update(
                    "update admin_game_provider_config set display_name = ?, status = ?, endpoint_url = ?, masked_secret = ?, incident_reference = ?, server_version = ?, updated_at = ? where tenant_id = ? and provider_id = ? and server_version = ?",
        ...
        jdbc.update(
            "insert into admin_game_provider_config_result(result_id, tenant_id, provider_id, query_fingerprint, evidence_reference, display_name, status, endpoint_url, masked_secret, incident_reference, server_version, occurred_at, idempotency_key, correlation_id, causation_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        jdbc.update(
            ...
            result.resultId, tenantId, "GAME_PROVIDER_CONFIG", result.serverTime
        )
        jdbc.update(
    ...
    private fun ResultSet.toResult(): Pair<String, GameProviderConfigResult> {
        val id = getObject("result_id", UUID::class.java)
        val provider = GameProviderConfig(
            providerId = getString("provider_id"),
            displayName = getString("display_name"),
            status = GameProviderStatus.valueOf(getString("status")),
        ...
        val res = GameProviderConfigResult(
            resultId = id,
            provider = provider,
