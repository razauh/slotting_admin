create table admin_aml_review_queue (
...
create index ix_aml_review_queue_state on admin_aml_review_queue(tenant_id, state, claim_expires_at);
create table admin_aml_review_result (
    ...
    foreign key (tenant_id, case_reference) references admin_aml_review_queue(tenant_id, case_reference),
    check (state in ('QUEUED','CLAIMED','APPROVED','REJECTED')),
    check (server_version >= 0)
);
