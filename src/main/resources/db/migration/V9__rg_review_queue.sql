create table admin_rg_review_queue (
...
create index ix_rg_review_queue_state on admin_rg_review_queue(tenant_id, state, claim_expires_at);
create table admin_rg_review_result (
    ...
    foreign key (tenant_id, case_reference) references admin_rg_review_queue(tenant_id, case_reference),
    check (state in ('QUEUED','CLAIMED','APPROVED','REJECTED')),
    check (server_version >= 0)
);
